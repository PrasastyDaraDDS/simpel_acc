...
@section('content')
    @component('components.breadcrumb')
        ...
        @endslot
    @endcomponent
    <form id="createproduct-form" action="{{ route('orders.update', $order->id) }}" method="POST" enctype="multipart/form-data"
                                    ...
                                    <div class="col-lg-6">
                                        <div class="mb-3">
                                            ...
                                            <div class="input-group has-validation mb-3">
                                                ...
                                                <input type="text" readonly name="buying_price" id="buying_price"
                            ...
                            <div class="table-responsive">
                                <table class="table table-striped table-nowrap align-middle mb-0">
                                    ...
                                    <tbody id="product-table">
                                        @foreach ($order->order_transactions as $ot)
                                            <td>{{ $ot->product->product_category_type->product_category->name }}</td>
                                            <td>{{ $ot->product->product_category_type->name }}</td>
                                            <td>{{ $ot->product->name }}</td>
                                            <td>{{ $ot->quantity }}</td>
                                            <td>{{ $ot->getRawPrice($order->order_type) }}</td>
                                            <td>{{ $ot->getProductTotalPrice($order->order_type) }}</td>
                                            <td>
                                                <button type="button"
                                                ...
                                                <select class="form-control form-control-sm" id="payment-select"
                                                    ...
                                                    @foreach ($methods as $method)
                                                        @if ($payment->payment_method_id == $method->id)
                                                            <option selected value="{{ $method->id }}">
                                                                {{ $method->name }}</option>
                                                        @else
                                            ...
                                            <td>
                                                <div class="input-group has-validation input-group-sm">
                                                    ...
                                                    <input type="number" class="form-control" name="amount[]"
                                                        placeholder="Enter amount" required value="{{old('amount',$payment->amount)}}">
                                                </div>
                                            </td>
...
@section('script')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            let order_type = @json($order->order_type);
